...
normalTail neg g0 =
  ...
  in 
    ...
    else if neg then (x - rNorm, g2) else (rNorm - x, g2)
...
sampleN :: (RandomGen g) 
        ...
        -> g 
        -> ([a], g)
sampleN n func g = if n >= 0 then sampleNInternal n func g else
  error $ "sampleN given negative n = " ++ show n 
sampleNInternal 0 _    g0 = ([], g0)
sampleNInternal n func g0 = (x : xs, g2) where 
  (x,  g1) = func g0 
  (xs, g2) = sampleNInternal (n - 1) func g1 

parSample :: (RandomGen g)
          => (g -> (a, g))
          -> [g]
          -> [(a, g)]
parSample = parMap rseq
-- Todo: Test vs rpar later, or rdeepseq

parSampleN  :: (RandomGen g) 
            => Int 
            -> (g -> (a, g)) 
            -> [g] 
            -> [([a], g)]
parSampleN n func = parMap rseq (sampleN n func)

---

--iterFrom :: (a -> a -> DTime -> b -> b) -> b -> SF a b

randomProcess :: (RandomGen g)
...
-- For discrete time Markov Processes, recommend using (Event a) as the input type (Equivalent to Maybe)

-- Example of the above in action:
-- A 2d brownian motion with variable standard deviation
brownianMotion2DVar :: (RandomGen g)
...
brownianMotion2DVar pos0 (ga, gb) = randomProcess update pos0 [ga, gb] 
  where update _ (s, [gx0, gy0]) dt (pos, _) = let [(dx, gx1), (dy, gy1)] = parSample sampleStandardNormal [gx0, gy0] in (pos + V2 (s * dt * dx) (s * dt * dy), [gx1, gy1])

brownianMotion2D  :: (RandomGen g)
                  => V2 Double
                  -> Double
                  -> (g, g)
                  -> SF () (V2 Double)
brownianMotion2D pos0 sd (ga, gb) = randomProcess update pos0 [ga, gb] 
  where update _ (_, [gx0, gy0]) dt (pos, _) = let [(dx, gx1), (dy, gy1)] = parSample sampleStandardNormal [gx0, gy0] in (pos + V2 (sd * dt * dx) (sd * dt * dy), [gx1, gy1])

brownianMotion3D  :: (RandomGen g)
                  => V3 Double
                  -> Double
                  -> (g, g, g)
                  -> SF () (V3 Double)
brownianMotion3D pos0 sd (ga, gb, gc) = randomProcess update pos0 [ga, gb, gc] 
  where update _ (_, [gx0, gy0, gz0]) dt (pos, _) = let [(dx, gx1), (dy, gy1), (dz, gz1)] = parSample sampleStandardNormal [gx0, gy0, gz0] in (pos + V3 (sd * dt * dx) (sd * dt * dy) (sd * dt * dz), [gx1, gy1, gz1])

-- Provide a steam of samples from a fixed distribution
sampleStream :: (RandomGen g)
             => (g -> (a, g)) -- Fixed distribution to sample
             -> a -- initial output
             -> g 
             -> SF () a 
sampleStream func a0 g0 = loopPre g0 (iterFrom sampleIter (a0, g0)) 
  where sampleIter _ (_, g) _ _ = func g 

-- Provide a stream of samples from a distribution given its parameters as input
sampleStreamParam :: (RandomGen g)
                  => (a -> g -> (b, g)) -- Parameterised distribution to sample
                  -> b -- initial output
                  -> g
                  -> SF a b
sampleStreamParam func b0 g0 = loopPre g0 (iterFrom sampleIterParam (b0, g0)) 
  where sampleIterParam _ (a, g) _ _ = func a g

-- Something of the form SF (Event a) b for modelling Discrete time Markov Processes

-- A way to convert Q and P matricies to IterFrom update functions,
